(function($) {
    ...
    $.widget("ui.tree", {
        options : {
            ...
            add_expand_all: true,
            add_expand_before : false,
            add_collapse_all: true,
            add_collapse_before : false,
        ...
        toggle: function(branch, method) {
            if(this.is_collapsed(branch)) {
                this.expand(branch,method);
            }
            else {
                this.collapse(branch,method);
            }
        },

        update: function() {
            this._build_tree_handlers();
            this._init_drag_n_drop();
        },
        ...
        _add_collapse_all_link: function() {
            ...
            if(this.options.add_collapse_before) {
                this.element.before(a_collapse_all);
            }
            else {
                this.element.after(a_collapse_all);
            }
        ...
        _add_expand_all_link: function() {
            ...
            if(this.options.add_expand_before) {
                this.element.before(a_expand_all);
            }
            else {
                this.element.after(a_expand_all);
            }
        ...
        _init_drag_n_drop: function() {
            ...
            if(widget.options.drag_start) {
                draggable_options.start = widget.options.drag_start;
            }
            $('li', widget.element).draggable(draggable_options);

            var drop_target;
            if(widget.options.drop_target === undefined) {
                drop_target = widget.options.handle_dragging;
            }
            else {
                drop_target = widget.options.drop_target;
            }
            ...
            var droppable_options = {
                ...
                drop: function(event, ui) {
                    $(this).removeClass(draggable_over_class);
                    var li = $(this).closest('li',widget.element[0]);
                    ...
                    if(widget.options.drop) {
                        widget.options.drop.call(this,event,ui);
                    }
//                    widget._trigger('drop', 0, {draggable: ui.draggable, droppable: li});
                }
            };
            if((widget.options.accept !== undefined)
            && (widget.options.accept !== null)) {
                droppable_options.accept = function(potential_target) {
                    return widget.options.accept.call(this
                                                     ,widget.options
                                                     ,potential_target);
                }
            }
            $(drop_target, widget.element).droppable(droppable_options);
        },
